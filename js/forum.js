        ...
        <div class="indicator flex flex-col lg:flex-row items-center justify-center lg:items-start">
          ...
          <div class="space-y-5 pl-5">
            ...
            <div class="flex gap-20 lg:gap-60">
              ...
              <div onclick="getTittle('${post.title}',${post.view_count})" class="btn bg-[#F2F2FF]">
...
const getTittle = (name,count) =>{
   const tittleContainer = document.getElementById('tittle-container');
   const div = document.createElement('div');
   let countNumber = parseInt(document.getElementById('count-number').innerText);
   countNumber++;
   document.getElementById('count-number').innerText = countNumber;
   div.innerHTML = `
   <div class="flex gap-4  items-center justify-center bg-white rounded-xl p-5 my-2">
   <h3 class="text-[#12132D] font-semibold font-mulish">${name}</h3>
   <h6 class="text-[#12132D99]"><i class="fa-regular fa-eye"></i>${count}</h6>
  </div>
   `;
   tittleContainer.appendChild(div);
}
